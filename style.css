html, body {
    ...
    width: 100%;
    height: 100%;
    background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); /* Add this line */
}
...
.container {
    ...
    min-height: 100vh;
    min-height: 100dvh;
    padding: min(20px, 2vh) min(20px, 2.5vw);
    position: relative;
...
.start-btn:hover, .submit-btn:hover, .next-round-btn:hover, .share-btn:hover {
    transform: translateY(-2px);
    box-shadow: 0 6px 20px rgba(0,0,0,0.3);
}
...
.keyboard {
    ...
    max-width: none;
    width: clamp(90vw, 95vw, 80vw); /* 90% minimum, 95% preferred, 80% maximum */
    margin-left: 50%;
    transform: translateX(-50%);
    flex-shrink: 0;
}
...
.key {
    background: rgba(255,255,255,0.9);
    border: none;
    padding: min(1.5vh, 15px) min(1vw, 8px);
    border-radius: 5px;
    cursor: pointer;
    font-weight: bold;
